...
## 2. Current Goal
* **현재 Sprint**: Sprint 2 (Supabase DDL 및 데이터베이스 구조체 셋업)
* **현재 작업 (Task)**: GitHub Repository 운영 규칙 반영 및 Sprint 2-1, 2-2 DDL 코드 리뷰 대기
* **완료 조건 (Definition of Done)**:
  ...
  3. 로컬 Git 저장소 초기화(`git init`) 및 브랜치(`develop`) 분할 커밋 생성 완료.
  4. ChatGPT PM의 코드 리뷰 및 승인 획득.
...
5. **Review/Embedding 엔티티의 완전 분리** (2026-06-26): 임베딩 모델 변경 및 유사도 검색 벡터 전략의 유연성을 위해 `customer_reviews`와 `review_embeddings` 테이블을 물리 분리함.

---

## 6. Pending Review (최우선 검토 목적)
* **Sprint 2-1 및 Sprint 2-2 DDL 스크립트 코드 리뷰**: `database/migrations/` 내의 01~11번 파일에 대한 Supabase PostgreSQL 16 호환성, Naming Convention 준수 여부 및 구조적 적정성 검토.
* **GitHub Repository 운영 가이드라인**: 새롭게 제정된 Branch 정책 및 PR 본문 명세서 규격 적정성 검토.

---

## 7. Next Action
* **ChatGPT (PM)**: 
  1. `develop` 브랜치에 작성된 11개 DDL 스크립트에 대해 통합 코드 리뷰를 수행하고 승인 여부를 결정해 주십시오.
  2. 승인 완료 시, 다음 마일스톤인 **[Sprint 2-3] Customer/JTBD Domain DDL 구현을 위한 세부 작업 지시서**를 작성해 주십시오.
