...
import { ProductStrategyInput, ProductStrategyResult } from './types.js';
import { buildPrompt } from './prompt.js';
import { parseResponse } from './parser.js';
import { validateResult } from './validator.js';
import { MAX_TOKENS, TEMPERATURE, PROMPT_VERSION } from './constants.js';

export class ProductStrategyService {
  constructor(private readonly aiProvider: AIProvider) {}

  async analyze(input: ProductStrategyInput, options?: Partial<AIRequestOptions>): Promise<ProductStrategyResult> {
    // 1. Prompt 생성
